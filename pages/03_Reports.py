...
st.set_page_config(
    page_title="Reports - Finteca AuditRep",
    page_icon="🏦",
    layout="wide"
)
...
.alert-medium   { background:#fff8e1; border-left:5px solid #f9a825; padding:12px 15px; border-radius:6px; margin:6px 0; }
...
<div class="finteca-header">
    ...
    <p>Reports · Fraud Detection · AI Investigation</p>
    <span class="finteca-badge">Module 3 — Reports</span>
</div>
...
with tabs[0]:
    ...
    sales  = load("sales");    purch = load("purchases")
    coll   = load("collections"); bank = load("banking")
    ret    = load("sales_returns"); inv = load("inventory")
    ...
    with col_a:
        if revenue > 0:
            fig = go.Figure(data=[
                go.Bar(name="Revenue",     x=["Summary"], y=[revenue],  marker_color="#1a237e"),
                go.Bar(name="COGS",        x=["Summary"], y=[cogs],     marker_color="#e53935"),
                go.Bar(name="Gross Profit",x=["Summary"], y=[gp],       marker_color="#2e7d32"),
            ])
    ...
    with col_b:
        if bank_in > 0 or bank_out > 0:
            fig2 = go.Figure(data=[
                go.Bar(name="Cash In",  x=["Banking"], y=[bank_in],  marker_color="#2e7d32"),
                go.Bar(name="Cash Out", x=["Banking"], y=[bank_out], marker_color="#c62828"),
            ])
...
with tabs[1]:
    ...
    if sales2.empty:
        ...
        total_rev  = sales2["net_amount"].sum()
        total_cogs = sales2["cost_of_goods"].sum()
        total_gp   = sales2["gross_profit"].sum()
        ret_total  = to_num(ret2.get("return_amount", pd.Series(dtype=float))).sum() if not ret2.empty else 0
        net_rev    = total_rev - ret_total
        gpm        = total_gp / total_rev * 100 if total_rev else 0

        m1,m2,m3,m4,m5 = st.columns(5)
        m1.metric("Gross Revenue", f"{total_rev:,.2f}")
        m2.metric("Returns",       f"{ret_total:,.2f}", delta_color="inverse")
        m3.metric("Net Revenue",   f"{net_rev:,.2f}")
        m4.metric("Gross Profit",  f"{total_gp:,.2f}")
        m5.metric("Gross Margin",  f"{gpm:.1f}%")

        if "item_description" in sales2.columns:
            by_item = sales2.groupby("item_description").agg(
            ...
            fig3 = px.bar(by_item.head(15), x="item_description", y="GrossProfit",
                          color="Margin_pct", title="Gross Profit by Item",
...
with tabs[2]:
    ...
    if bank3.empty:
        ...
        tot_in   = bank3["credit"].sum()
        tot_out  = bank3["debit"].sum()
        net_c    = tot_in - tot_out
        tot_col  = to_num(coll3.get("amount", pd.Series(dtype=float))).sum() if not coll3.empty else 0

        m1,m2,m3,m4 = st.columns(4)
        m1.metric("Total Inflows",   f"{tot_in:,.2f}")
        m2.metric("Total Outflows",  f"{tot_out:,.2f}", delta_color="inverse")
        m3.metric("Net Cash",        f"{net_c:,.2f}")
        m4.metric("Total Collected", f"{tot_col:,.2f}")

        try:
            bank3["date"] = pd.to_datetime(bank3["date"], errors="coerce")
            monthly = bank3.groupby(bank3["date"].dt.to_period("M")).agg(
                Inflows=("credit","sum"), Outflows=("debit","sum")
            ).reset_index()
            monthly["date"] = monthly["date"].astype(str)
            monthly["Net"]  = monthly["Inflows"] - monthly["Outflows"]
            fig4 = px.line(monthly, x="date", y=["Inflows","Outflows","Net"],
                           title="Monthly Cash Flow",
                           color_discrete_sequence=["#2e7d32","#c62828","#1a237e"])
...
with tabs[3]:
    ...
    coll4  = load("collections");  bank4  = load("banking")
    sales4 = load("sales");        ret4   = load("sales_returns")
    inv5   = load("inventory")

    # 1. Unbanked collections
    if not coll4.empty and not bank4.empty and "credit" in bank4.columns:
        ...
        if not unbanked.empty:
            alerts.append({"severity":"🔴 HIGH","type":"UNBANKED COLLECTIONS",
                "description":f"{len(unbanked)} collections totalling {unbanked['amount'].sum():,.2f} not in bank",
                "count":len(unbanked),"data":unbanked})
    ...
    if not sales4.empty and "invoice_number" in sales4.columns:
        ...
        if not dups.empty:
            alerts.append({"severity":"🔴 HIGH","type":"DUPLICATE INVOICES",
                "description":f"{len(dups)} duplicate invoice numbers detected",
                "count":len(dups),"data":dups})

    # 3. Returns without original sale
    if not ret4.empty and not sales4.empty:
        if "original_invoice" in ret4.columns and "invoice_number" in sales4.columns:
            ...
            if not orphans.empty:
                alerts.append({"severity":"🔴 HIGH","type":"RETURN FRAUD",
                    "description":f"{len(orphans)} returns with no matching original sale",
                    "count":len(orphans),"data":orphans})

    # 4. Excessive discounts
    if not sales4.empty and "discount" in sales4.columns and "net_amount" in sales4.columns:
        sales4["discount"]   = to_num(sales4["discount"])
        sales4["net_amount"] = to_num(sales4["net_amount"])
        ...
        if not high_disc.empty:
            alerts.append({"severity":"🟠 MEDIUM","type":"EXCESSIVE DISCOUNTS",
                "description":f"{len(high_disc)} sales with >20% discount — total: {high_disc['discount'].sum():,.2f}",
                "count":len(high_disc),"data":high_disc})
    ...
    if not inv5.empty and "variance_value" in inv5.columns:
        ...
        if not major.empty:
            alerts.append({"severity":"🟠 MEDIUM","type":"INVENTORY VARIANCE",
                "description":f"{len(major)} items with variance >100 — total: {major['variance_value'].sum():,.2f}",
                "count":len(major),"data":major})

    high_alerts = [a for a in alerts if "HIGH"   in a["severity"]]
    med_alerts  = [a for a in alerts if "MEDIUM" in a["severity"]]
    m1, m2, m3  = st.columns(3)
    m1.metric("🔴 High Risk",   len(high_alerts))
    m2.metric("🟠 Medium Risk", len(med_alerts))
    m3.metric("Total Alerts",   len(alerts))
    ...
    if not alerts:
        ...
        for alert in alerts:
            sev = alert["severity"]
            css = "alert-critical" if "HIGH" in sev else "alert-high"
...
with tabs[4]:
    ...
    s5 = load("sales"); c5 = load("collections")
    r5 = load("sales_returns"); b5 = load("banking"); i5 = load("inventory")
    ...
    gp5   = rev5 - cogs5
    gpm5  = gp5 / rev5 * 100 if rev5 else 0
    ret_r = ret5v / rev5 * 100 if rev5 else 0
    col_r = col5v / rev5 * 100 if rev5 else 0

    avg_inv5 = 0
    if not i5.empty:
        ov = to_num(i5.get("opening_value", pd.Series(dtype=float))).sum()
        cv = to_num(i5.get("closing_value", pd.Series(dtype=float))).sum()
        avg_inv5 = (ov + cv) / 2
    ...
    ratios = [
        ("Gross Profit Margin",   f"{gpm5:.1f}%",          "Higher is better. Typical: 30-50%"),
        ("COGS to Revenue",       f"{cogs5/rev5*100 if rev5 else 0:.1f}%", "Lower is better"),
        ("Return Rate",           f"{ret_r:.2f}%",          "Healthy below 5%"),
        ("Collection Rate",       f"{col_r:.1f}%",          "Should be near 100%"),
        ("Inventory Turnover",    f"{inv_turn:.2f}x",       "Higher = faster moving stock"),
        ("Cash vs Revenue",       f"{bnk5/rev5*100 if rev5 else 0:.1f}%", "Should be near 100%"),
        ("Avg Transaction Value", f"{avg_tx:,.2f}",         "Average sale size"),
        ("Total Discounts Given", f"{disc5:,.2f}",          "Monitor for abuse"),
    ]
    rcols = st.columns(4)
...
with tabs[5]:
    ...
    if not API_KEY:
        ...
        quick_qs   = [
            "Summarise the overall financial health",
            "What are the biggest fraud risks?",
            "Which items have inventory losses?",
            "What collections are not banked?",
        ]
        for qcol, q in zip(quick_cols, quick_qs):
        ...
        if "pending_q" in st.session_state:
            ...
            for t in ["sales","purchases","banking","collections","sales_returns","swap_deals","inventory"]:
                df = load(t)
                if not df.empty:
                    context[t] = {"rows": len(df), "columns": list(df.columns),
            ...
            with st.spinner("🤖 Investigating..."):
                try:
                    resp = client.chat.completions.create(
                        model="gpt-4o",
                        messages=[
                            {"role":"system","content":"You are Finteca AuditRep AI forensic accountant. Be specific, cite numbers, flag risks."},
        ...
        if st.button("📄 Generate Executive Report", type="primary", use_container_width=True):
            with st.spinner("Generating executive report..."):
                try:
                    ctx = {t: {"rows": len(load(t))} for t in
                           ["sales","purchases","banking","collections","inventory"]}
                    resp2 = client.chat.completions.create(
                        model="gpt-4o",
                        messages=[
                            {"role":"system","content":"You are a CFO writing an executive audit report for Finteca AuditRep."},
                            {"role":"user","content":f"""Write a comprehensive executive financial report.
...
## 7. Recommendations"""}
                    ...
                    st.download_button("📥 Download Report", report,
                        f"finteca_report_{datetime.now().strftime('%Y%m%d')}.txt","text/plain")
                except Exception as e:
                    st.error(f"Error: {e}")
