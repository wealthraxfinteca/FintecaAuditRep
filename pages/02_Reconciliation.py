...
st.set_page_config(
    page_title="Reconciliation - Finteca AuditRep",
    page_icon="🏦",
    layout="wide"
)
...
<div class="finteca-header">
    ...
    <span class="finteca-badge">Module 2 — Reconciliation</span>
</div>
...
# ── TAB 1: Bank vs Collections ────────────────────────────
with tabs[0]:
    st.markdown(
        '<div class="section-header">🏦 Bank Statement vs Collections</div>',
        unsafe_allow_html=True
    )
    st.info(
        "Matches every collection against actual bank deposits. "
        "Unmatched items require immediate investigation."
    )
    ...
    if coll.empty:
        ...
        for _, r in coll.iterrows():
            ...
            if not matched.empty:
                ...
                if days_diff > 3:
                    status   = f"⚠️ LATE BANKING ({days_diff} days)"
                else:
                    status   = "✅ BANKED"
                unbanked = 0.0
            else:
        ...
        if not df_r.empty:
            tot      = df_r["Amount"].sum()
            banked   = df_r[
                df_r["Status"].str.contains("BANKED", na=False) &
                ~df_r["Status"].str.contains("NOT", na=False)
            ]["Amount"].sum()
            ...
            fig = px.pie(
                ...
                names="Status",
                values="Amount",
                title="Collections Banking Status",
                color_discrete_sequence=[
                    "#2e7d32", "#c62828", "#f57f17", "#1a237e"
                ],
            )
            st.plotly_chart(fig, use_container_width=True)

            def highlight(row):
                if "NOT BANKED" in str(row["Status"]):
                    return ["background-color:#ffebee"] * len(row)
                elif "LATE" in str(row["Status"]):
                    return ["background-color:#fff8e1"] * len(row)
                return ["background-color:#e8f5e9"] * len(row)

            st.dataframe(
                df_r.style.apply(highlight, axis=1),
            ...
            st.download_button(
                ...
                f"bank_recon_{datetime.now().strftime('%Y%m%d')}.csv",
                "text/csv",
            )

# ── TAB 2: Purchases vs Payments ──────────────────────────
with tabs[1]:
    st.markdown(
        '<div class="section-header">🛒 Purchases vs Bank Payments</div>',
        unsafe_allow_html=True,
    )
    ...
    if purch.empty:
        ...
        purch["total_cost"] = to_num(
            purch.get("total_cost", pd.Series(dtype=float))
        )
        debits = pd.DataFrame()
        ...
        for _, r in purch.iterrows():
            ...
            if not debits.empty:
                if ref and "description" in debits.columns:
                    rm = debits[
                        debits["description"]
                        .astype(str)
                        .str.contains(ref, case=False, na=False)
                    ]
                    if not rm.empty:
                        matched2 = rm
                if matched2.empty:
            ...
            bank_ref = (
                str(matched2.iloc[0].get("reference", "") or "")
            ...
            rows2.append({
                ...
                "Bank Ref": bank_ref,
                "Status": status2,
                "Outstanding": 0 if not matched2.empty else amt,
            })
        ...
        m4.metric(
            "Payment Rate",
            f"{paid_p / tot_p * 100 if tot_p else 0:.1f}%",
        )
        st.dataframe(df_p, use_container_width=True, height=380)
        st.download_button(
            "📥 Download",
            df_p.to_csv(index=False),
            f"purchase_recon_{datetime.now().strftime('%Y%m%d')}.csv",
            "text/csv",
        )

# ── TAB 3: Sales vs Collections ───────────────────────────
with tabs[2]:
    st.markdown(
        '<div class="section-header">💰 Sales vs Collections</div>',
        unsafe_allow_html=True,
    )
    sales3 = load("sales")
    ...
    if sales3.empty:
        ...
        sales3["net_amount"] = to_num(
            sales3.get("net_amount", pd.Series(dtype=float))
        )
        rows3 = []
        for _, r in sales3.iterrows():
            ...
            if not coll3.empty and "invoice_reference" in coll3.columns and inv:
                mc = coll3[
                    coll3["invoice_reference"]
                    .astype(str)
                    .str.contains(inv, case=False, na=False)
                ]
                if not mc.empty:
                    collected = float(
                        to_num(mc["amount"]).sum()
                    )
            bal = amt - collected
            if abs(bal) < 0.01:
                st3 = "✅ FULLY COLLECTED"
        ...
        m4.metric(
            "Collection Rate",
            f"{col_s / tot_s * 100 if tot_s else 0:.1f}%",
        )
        st.dataframe(df_s, use_container_width=True, height=380)
        st.download_button(
            "📥 Download",
            df_s.to_csv(index=False),
            f"sales_recon_{datetime.now().strftime('%Y%m%d')}.csv",
            "text/csv",
        )
...
with tabs[3]:
    st.markdown(
        '<div class="section-header">📦 Inventory Reconciliation</div>',
        unsafe_allow_html=True,
    )
    inv4   = load("inventory")
    sales4 = load("sales")
    ...
    if inv4.empty:
        ...
        for _, item in inv4.iterrows():
            ...
            if not sales4.empty and "item_description" in sales4.columns:
                ms = sales4[
                    sales4["item_description"]
                    .astype(str)
                    .str.lower() == desc.lower()
                ]
                sales_qty += float(
                    to_num(ms.get("quantity", pd.Series(dtype=float))).sum()
                )
            if not purch4.empty and "item_description" in purch4.columns:
                mp = purch4[
                    purch4["item_description"]
                    .astype(str)
                    .str.lower() == desc.lower()
                ]
                purch_qty += float(
                    to_num(mp.get("quantity", pd.Series(dtype=float))).sum()
                )
            if not ret4.empty and "item_description" in ret4.columns:
                mr = ret4[
                    ret4["item_description"]
                    .astype(str)
                    .str.lower() == desc.lower()
                ]
                ret_qty += float(
                    to_num(
                        mr.get("quantity_returned", pd.Series(dtype=float))
                    ).sum()
                )

            expected = (
                opening + purch_qty - sales_qty
                + ret_qty - swap_out + swap_in + adj
            )
            ...
            rows4.append({
                ...
                "Swap Out": swap_out,
                "Swap In": swap_in,
                "Expected Closing": round(expected, 2),
        ...
        if not vd.empty:
            fig4 = px.bar(
                ...
                y="Variance Value",
                color="Status",
                title="Inventory Variance by Item",
                color_discrete_map={
                    "✅ BALANCED": "#2e7d32",
                    "🟡 MINOR":    "#f9a825",
                    "🟠 MODERATE": "#e65100",
        ...
        st.download_button(
            "📥 Download",
            df_i.to_csv(index=False),
            f"inventory_recon_{datetime.now().strftime('%Y%m%d')}.csv",
            "text/csv",
        )
...
with tabs[4]:
    st.markdown(
        '<div class="section-header">🔄 Swap Deal Verification</div>',
        unsafe_allow_html=True,
    )
    ...
    if swaps.empty:
        ...
        swaps["difference_amount"] = to_num(
            swaps.get("difference_amount", pd.Series(dtype=float))
        )
        swaps["customer_paid"] = to_num(
            swaps.get("customer_paid", pd.Series(dtype=float))
        )
        swaps["variance"] = swaps["customer_paid"] - swaps["difference_amount"]

        m1, m2, m3 = st.columns(3)
        m1.metric(
            "Total Difference Due",
            f"{swaps['difference_amount'].sum():,.2f}",
        )
        m2.metric(
            "Total Collected",
            f"{swaps['customer_paid'].sum():,.2f}",
        )
        m3.metric(
            "Variance",
            f"{swaps['variance'].sum():,.2f}",
            delta_color="inverse",
        )
...
with tabs[5]:
    st.markdown(
        '<div class="section-header">↩️ Sales Returns Reconciliation</div>',
        unsafe_allow_html=True,
    )
    ...
    if ret6.empty:
        ...
        ret6["return_amount"] = to_num(
            ret6.get("return_amount", pd.Series(dtype=float))
        )
        orphan    = 0
        restocked = 0
        ...
        if (
            not sales6.empty
            and "original_invoice" in ret6.columns
            and "invoice_number" in sales6.columns
        ):
            orphan = len(
                ret6[
                    ~ret6["original_invoice"].isin(
                        sales6["invoice_number"]
                    )
                ]
            )
        ...
        if orphan > 0:
            st.markdown(
                ...
                f"🚨 {orphan} returns have no matching original sale "
                f"— investigate immediately.</div>",
                unsafe_allow_html=True,
            )
